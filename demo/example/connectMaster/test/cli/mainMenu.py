...
from example.connectMaster.test.cli.deviceMenu import DeviceMenu
from example.connectMaster.test.cli.asyncMenu import AsyncMenu
from example.connectMaster.test.cli.acceptMenu import AcceptMenu
...
class MainMenu:
  ...
  connectMasterSvc = None
  gatewayID = None

  def __init__(self, connectMasterSvc, gatewayID):
    self.connectMasterSvc = connectMasterSvc
    self.gatewayID = gatewayID

    self.devMenu = DeviceMenu(connectMasterSvc, gatewayID)
    self.asyncMenu = AsyncMenu(connectMasterSvc, gatewayID)
    self.acceptMenu = AcceptMenu(connectMasterSvc, gatewayID)
  ...
  def searchDevice(self):
    ...
    try:
      devList = self.connectMasterSvc.searchDevice(self.gatewayID, SEARCH_TIMEOUT)
  ...
  def connect(self):
    try:
      ...
      devID = self.connectMasterSvc.connect(self.gatewayID, connInfo)
